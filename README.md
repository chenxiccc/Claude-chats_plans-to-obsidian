...
- 生成结构化 Markdown，含 YAML frontmatter（`created` / `modified` datetime）
- 按 Round 编号、带时间戳
...
- 始终取首条用户消息作为标题，按 East Asian Width 截断至 40 单位宽度（CJK/全角计 2，ASCII 计 1）
- 自动清洗 Markdown 格式符和路径不安全字符
- VS Code 标签降级为 frontmatter `label:` 元数据，不影响标题
...
内容包含：
- YAML frontmatter（`created` 和 `modified`，datetime 类型）
...
session-to-obsidian.py
    ...
    ├─ 过滤系统噪音
    ├─ 提取首条用户消息为标题（East Asian Width 截断）
...
**已归档的会话更新了怎么办？**
脚本通过比较 JSONL 和输出文件的 mtime 检测更新。如果 JSONL 比输出文件新，会自动重新生成。
...
- Generates structured Markdown with YAML frontmatter (`created` / `modified` datetime)
...
- Always uses the first user message as the title, truncated by East Asian Width (CJK/fullwidth = 2, ASCII = 1, max 40)
- Auto-cleans Markdown formatting and path-unsafe characters
- VS Code labels are demoted to frontmatter `label:` metadata, not used for titles
...
Contents:
- YAML frontmatter (`created` and `modified`, datetime type)
...
session-to-obsidian.py
    ...
    ├─ Filter system noise
    ├─ Extract first user message as title (East Asian Width truncation)
    ├─ Generate structured Markdown (with frontmatter)
    ├─ mtime incremental detection, deduplication
    ▼
~/Obsidian/Project/claude/session/
  ├─ -Users-admin/*.md
  ├─ -Users-admin-Documents-Tech-project-fns/*.md
...
The script compares JSONL and output file mtime. If the JSONL is newer, it automatically regenerates.
