...
- 生成结构化 Markdown，含 YAML frontmatter（`created`/`modified`/`cwd`/`label`）
- 按 Round 编号组织，带北京时间戳
...
- 会话笔记中以 `## 📋 [[plans/文件名]]` wikilink 精准引用对应版本
- 同一 plan 被覆写时旧版本保留，前后引用不混淆
...
cwd: '/Users/admin/Documents/Tech/project/session-to-obsidian'
---
...
## 📋 [[plans/修复登录页 token 验证逻辑 20260521-115130.md]]
...
| 变量 | 说明 |
|------|------|
| `OBSIDIAN_DIR` | Markdown 输出根目录，脚本自动在其下创建项目子目录 |
| `TRANSCRIPTS_DIR` | Claude Code 转录文件父目录 |
| `CHINA_TZ` | 时间戳转换时区 |
...
- Tool calls collapsed in `<details>` tags
...
- Session notes reference the correct version via `## 📋 [[plans/filename]]` wikilinks
- Old versions preserved when a plan is overwritten
